...
function createExternalPackageIconLoader(packageName, autoInstall = false) {
  let scope;
  let collection;
  const collections = {};
...
function createCustomIconLoader(scope, collection, autoInstall) {
  const iconSetPromise = loader_fs.loadCollectionFromFS(collection, autoInstall, scope);
  return async (icon) => {
